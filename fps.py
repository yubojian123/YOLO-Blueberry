...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=list,
                        default=[
                            './exps/ab/weights/best.pt',
                            './exps/a/weights/best.pt',
                            './exps/b/weights/best.pt',
                            './exps/raw2/weights/best.pt',
                        ], help='trained weights path')
    parser.add_argument('--batch', type=int, default=1, help='total batch size for all GPUs')
    parser.add_argument('--imgs', nargs='+', type=int,
                        default=[640, 640], help='[height, width] image sizes')
    ...
    for weights in opt.weights:
        ...
        print(f'model weights:{weights} size:{get_weight_size(weights)}M (bs:{opt.batch})Latency:{infer_time_per_image:.5f}s +- {std_time:.5f}s fps:{1 / infer_time_per_image:.1f}')
        print()
